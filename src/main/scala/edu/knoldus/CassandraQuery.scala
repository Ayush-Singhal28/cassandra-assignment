...
object CassandraQuery extends App with CassandraProvider {
  ...
  creatingEmployeeTable(cassandraSession)
  insertingDataToTable(cassandraSession)
  fetchingDatabyId(cassandraSession, empId)
  updatingData(cassandraSession, empId1, empSalary1)
  gettingDataBySalary(cassandraSession, empSalary, empId1)
  gettingDataByCity(cassandraSession, empCity)
  deletingDataByCity(cassandraSession, empCity)
